...
public class AttendancePathActivity extends Activity implements View.OnClickListener{
    ImageButton ibBack;
    MapView mMapView;
    BaiduMap mBaiduMap;
    ...
    /*String SDPATH_1 ="/sdcard/headPic_1.jpg";
    String SDPATH_2 ="/sdcard/headPic_2.jpg";*/
    ...
    double rate;
    Intent intent;
    String projectId = "83", projectName = "" , humenType = "外勤人员";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(PreferenceUtil.getIsBattery() && PreferenceUtil.getIsAutoStart()){
            ...
            Intent intent = new Intent(AttendancePathActivity.this,PermissionsSettingsActivity.class);
            startActivity(intent);
        }
        intent = getIntent();
       progressDialog = DialogUtil.createProgressDialog(AttendancePathActivity.this, "初始化中，请稍后...");
        initHead();
        // 地图初始化
        ...
        switch (v.getId()){
            ...
            case R.id.btnStart:
                if(isHeadImg){
                    ...
                    Toast.makeText(AttendancePathActivity.this,"您无底图请联系管理员！",Toast.LENGTH_SHORT).show();
                }
                //JSONArray listArrays= JSONArray.fromObject(list);
                break;
            case R.id.btnEnd:
                new AlertDialog.Builder(AttendancePathActivity.this).setTitle("提示").setMessage("是否确认上传路径？")
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                progressDialog = DialogUtil.createProgressDialog(AttendancePathActivity.this, "正在上传中，请稍后...");
                                insertLocation(false);
                                new  insertDatTrajectoryAsyncTask().execute();
    ...
    public void faceRec(){
        progressDialog = DialogUtil.createProgressDialog(AttendancePathActivity.this, "人脸监测中，请稍后...");
        progressDialog.show();
        FaceRecognition.getInstance().startFaceMatching(AttendancePathActivity.this,Constants.RSA_KEY, Constants.PRIVATE_RSA, imageOneBase64, imageTwoBase64, new StringCallback() {
            ...
            public void onError(Call call, Exception e, int i) {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(AttendancePathActivity.this);
                // 设置提示框的标题
                builder.setTitle("提示：").
                        // 设置提示框的图标
    ...
    class getHeadImageAsyncTask extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            return new NetworkApi().getHeadImage(AttendancePathActivity.this);
        }
        ...
        protected void onPostExecute(final String result) {
            ...
            if (result == null) {
                progressDialog.dismiss();
                isHeadImg = false;
                //Toast.makeText(AttendancePathActivity.this,"暂无头像信息",Toast.LENGTH_SHORT).show();
                return;
            }else{
                new Thread(){
                    public void run(){
    ...
    class upLoadPhotoAsyncTask extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            return new NetworkApi().upLoadPhoto(AttendancePathActivity.this,projectId,projectName,"其他",humenType,imageTwoBase64);
        }
        ...
        protected void onPostExecute(String result) {
            ...
            progressDialog.dismiss();
            if (result == null) {
                Toast.makeText(AttendancePathActivity.this,"信息上传失败，请重试！",Toast.LENGTH_SHORT).show();
                ...
                if (rate >= 0.6) {
                    //progressDialog.dismiss();
                    Toast.makeText(AttendancePathActivity.this,"考勤记录已开启！",Toast.LENGTH_SHORT).show();
                    PreferenceUtil.saveAttendandcePath(true);
    ...
    class insertDatTrajectoryAsyncTask extends AsyncTask<String,String , String> {
        ...
        protected String doInBackground(String... params) {
            ...
            return new NetworkApi().insertDatTrajectory(AttendancePathActivity.this,projectId,gson.toJson(list), PreferenceUtil.getOfficeCapId(),StrartTime,EndTime);
        }
        ...
        protected void onPostExecute(String result) {
            ...
            if (result == null ) {
                ...
                Toast.makeText(AttendancePathActivity.this,"上传成功！",Toast.LENGTH_SHORT).show();
                PreferenceUtil.saveAttendandcePath(false);
                btnEnd.setEnabled(false);
    ...
    private void insertLocation(boolean isClearSharePreference){
        try{
            ...
            Log.e("11111",MyApplication.attendancePathDao.loadPhysiological().get(MyApplication.attendancePathDao.loadPhysiological().size()-1).toString());
