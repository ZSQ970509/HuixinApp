...
public class LadderControlAddHumenSearchProjiect extends Activity implements OnClickListener {
	...
	private void initView() {
		// TODO Auto-generated method stub
		final String title = getIntent().getStringExtra("classtitle");
		...
		mProjectNameList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				Intent intent = new Intent(LadderControlAddHumenSearchProjiect.this,
						LadderControlAddHumenMessage.class);
				intent.putExtra("type", getIntent().getStringExtra("type"));
				intent.putExtra("buttonName", getIntent().getStringExtra("buttonName"));
	...
	private class GetDictByDirTypeAsyncTask extends AsyncTask<Void, Void, ArrayList<MaintenanceTypeItem>> {
		...
		protected void onPostExecute(final ArrayList<MaintenanceTypeItem> data) {
			...
			try {
				popup = new TitlePopup(LadderControlAddHumenSearchProjiect.this, safeSpinner.getWidth(),
			...
			for (int i = 0; i < data.size(); i++) {
				...
				popup.addAction(new ActionItem(LadderControlAddHumenSearchProjiect.this, list.get(i).toString(),
						R.drawable.account));
	...
	private void searchProject() {
		...
		if (TextUtils.isEmpty(name)) {
			ToastHelp.showToast(LadderControlAddHumenSearchProjiect.this, "关键字不能为空！");
			return;
		}
		mData.clear();
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				/*NetworkApi.getProjectListForBindFailSev(LadderControlAddHumenSearchProjiect.this, name, mKeytype,
						new INetCallback() {
							@Override
								...
								if (value) {
									...
									if (count == 0) {
										mHandler.post(new Runnable() {
											...
											public void run() {
												dialog.cancel();
												ToastHelp.showToast(LadderControlAddHumenSearchProjiect.this,
														"未搜索到工程数据！");
											}
										});
										return;
									...
									final ArrayAdapter<String> mWorkOrderAdapter = new ArrayAdapter<String>(
											LadderControlAddHumenSearchProjiect.this, R.layout.simple_spinner_item,
											dataList);
									...
									mHandler.post(new Runnable() {
										...
										public void run() {
											dialog.cancel();
											ToastHelp.showToast(LadderControlAddHumenSearchProjiect.this, "获取工程名称失败！");
										}
									});
								}
							}
				...
				NetworkApi.queryProjectData(LadderControlAddHumenSearchProjiect.this, name,
						new INetCallback() {
							@Override
							public void onCallback(boolean value, String result) {
								if (value) {
									...
									if (count == 0) {
										mHandler.post(new Runnable() {
											...
											public void run() {
												dialog.cancel();
												ToastHelp.showToast(LadderControlAddHumenSearchProjiect.this, "未搜索到工程数据！");
											}
										});
										return;
									}
									...
									final ArrayAdapter<String> adapter = new ArrayAdapter<String>(
											LadderControlAddHumenSearchProjiect.this, R.layout.simple_spinner_item, dataList);
