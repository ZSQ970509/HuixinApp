...
public class PowerFailDeviceSearchActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_powerfaildevice_search);
	...
	private void initView() {
		...
		mProjectNameList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				Intent intent = new Intent(PowerFailDeviceSearchActivity.this, PowerFailDeviceMainActivity.class);
	...
	private class GetDictByDirTypeAsyncTask extends AsyncTask<Void, Void, ArrayList<MaintenanceTypeItem>> {
		...
		protected void onPostExecute(final ArrayList<MaintenanceTypeItem> data) {
			...
			try {
				popup = new TitlePopup(PowerFailDeviceSearchActivity.this, safeSpinner.getWidth(),
			...
			for (int i = 0; i < data.size(); i++) {
				...
				popup.addAction(
						new ActionItem(PowerFailDeviceSearchActivity.this, list.get(i).toString(), R.drawable.account));
	...
	private void searchProject() {
		...
		if (TextUtils.isEmpty(name)) {
			ToastHelp.showToast(PowerFailDeviceSearchActivity.this, "关键字不能为空！");
			return;
		}
		mData.clear();
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				NetworkApi.getProjectListForOutSideCapture(PowerFailDeviceSearchActivity.this, name, mKeytype,
						new INetCallback() {
							@Override
								...
								if (value) {
									...
									if (count == 0) {
										mHandler.post(new Runnable() {
											...
											public void run() {
												dialog.cancel();
												ToastHelp.showToast(PowerFailDeviceSearchActivity.this, "未搜索到工程数据！");
											}
										});
										return;
									}
									...
									final ArrayAdapter<String> adapter = new ArrayAdapter<String>(
											PowerFailDeviceSearchActivity.this, R.layout.simple_spinner_item, dataList);
									...
									mHandler.post(new Runnable() {
										...
										public void run() {
											dialog.cancel();
											ToastHelp.showToast(PowerFailDeviceSearchActivity.this, "获取工程名称失败！");
										}
									});
								}
							}
