...
public class DatBasicLevelSearch extends Activity implements OnClickListener {
	...
	private void initView() {
		...
		mProjectNameList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				Intent intent = new Intent(DatBasicLevelSearch.this,
						DatBasicLevelMessage.class);
	...
	private class GetDictByDirTypeAsyncTask extends AsyncTask<Void, Void, ArrayList<MaintenanceTypeItem>> {
		...
		protected void onPostExecute(final ArrayList<MaintenanceTypeItem> data) {
			...
			try {
				popup = new TitlePopup(DatBasicLevelSearch.this, safeSpinner.getWidth(),
			...
			for (int i = 0; i < data.size(); i++) {
				...
				popup.addAction(new ActionItem(DatBasicLevelSearch.this, list.get(i).toString(),
						R.drawable.account));
	...
	private void searchProject() {
		...
		if (TextUtils.isEmpty(name)) {
			ToastHelp.showToast(DatBasicLevelSearch.this, "关键字不能为空！");
			return;
		}
		mData.clear();
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				NetworkApi.queryProjectData(DatBasicLevelSearch.this, name,
						new INetCallback() {
							@Override
							public void onCallback(boolean value, String result) {
								if (value) {
									...
									if (count == 0) {
										mHandler.post(new Runnable() {
											...
											public void run() {
												dialog.cancel();
												ToastHelp.showToast(DatBasicLevelSearch.this, "未搜索到工程数据！");
											}
										});
										return;
									}
									...
									final ArrayAdapter<String> adapter = new ArrayAdapter<String>(
											DatBasicLevelSearch.this, R.layout.simple_spinner_item, dataList);
									...
									mHandler.post(new Runnable() {
										...
										public void run() {
											dialog.cancel();
											ToastHelp.showToast(DatBasicLevelSearch.this, "获取工程名称失败！");
										}
									});
								}
							}
