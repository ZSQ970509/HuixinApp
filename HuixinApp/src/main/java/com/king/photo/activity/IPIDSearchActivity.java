 ...
 * 监管影像采集-项目搜索界面 IP/ID
 */
public class IPIDSearchActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ipid_search);
	...
	private void initView() {
		...
		mProjectNameList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				Intent intent = new Intent(IPIDSearchActivity.this, IPIDQueryActivity.class);
	...
	private class GetDictByDirTypeAsyncTask extends AsyncTask<Void, Void, ArrayList<MaintenanceTypeItem>> {
		...
		protected void onPostExecute(final ArrayList<MaintenanceTypeItem> data) {
			...
			try {
				popup = new TitlePopup(IPIDSearchActivity.this, safeSpinner.getWidth(), LayoutParams.WRAP_CONTENT);
			...
			for (int i = 0; i < data.size(); i++) {
				...
				popup.addAction(new ActionItem(IPIDSearchActivity.this, list.get(i).toString(), R.drawable.account));
	...
	private void searchProject() {
		...
		if (TextUtils.isEmpty(name)) {
			ToastHelp.showToast(IPIDSearchActivity.this, "关键字不能为空！");
			return;
		}
		mData.clear();
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				NetworkApi.getProjectListForOutSideCapture(IPIDSearchActivity.this, name, mKeytype, new INetCallback() {
					@Override
						...
						if (value) {
							...
							if (count == 0) {
								mHandler.post(new Runnable() {
									...
									public void run() {
										dialog.cancel();
										ToastHelp.showToast(IPIDSearchActivity.this, "未搜索到工程数据！");
									}
								});
								return;
							}
							...
							final ArrayAdapter<String> adapter = new ArrayAdapter<String>(IPIDSearchActivity.this,
							...
							mHandler.post(new Runnable() {
								...
								public void run() {
									dialog.cancel();
									ToastHelp.showToast(IPIDSearchActivity.this, "获取工程名称失败！");
								}
							});
						}
					}
